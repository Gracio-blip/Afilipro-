...
const taskMeta: Record<string, { icon: string; diff: string; diffColor: string; tip: string; calc: string }> = {
  ...
  telegram: {
    ...
    calc: "Récompense fixe : 150 FCFA",
  },
  tiktok: {
    icon: "🎵",
    diff: "🟡 Moyenne",
    diffColor: "bg-amber-50 text-amber-700 border border-amber-200",
    tip: "Suis le compte partenaire TikTok et reste abonné. Vérification admin.",
    calc: "Récompense fixe : 150 FCFA",
  },
};
...
export default async function MissionsPage() {
  ...
  // Exclure le spin des tâches (Lucky Spin a sa propre page)
  const allTasks = (await db.select().from(tasks).where(eq(tasks.isActive, true)))
    .filter((t) => t.type !== "spin");
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-[24px] bg-[#0B1120] px-5 py-6 text-white">
        ...
        {!isActive && (
          <p className="mt-3 rounded-xl bg-rose-500/20 px-3 py-2 text-[12px] font-bold text-rose-300">
            ⛔ Déposez 2 500 FCFA pour accéder aux tâches
          </p>
        )}
      </div>
          ...
          return (
            <div key={task.id} className={`rounded-[20px] border border-slate-200 bg-white p-4 afili-card ${done ? "opacity-60" : ""}`}>
              <div className="flex items-start gap-4">
                <div className="grid h-14 w-14 shrink-0 place-items-center rounded-2xl bg-slate-900 text-[26px]">
                  {meta.icon}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  <span className={`mt-2 inline-block rounded-lg px-2 py-0.5 text-[10px] font-black ${meta.diffColor}`}>
                    {meta.diff}
                  </span>
                  <div className="mt-2 rounded-xl bg-slate-50 px-3 py-2">
                    <p className="text-[11px] font-black text-slate-600">📊 {meta.calc}</p>
                    <p className="text-[10px] text-slate-400 mt-0.5">{meta.tip}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4">
                {!isActive ? (
                  <div className="flex items-center justify-center rounded-2xl bg-slate-100 py-3 text-[13px] font-black text-slate-400">
                    🔒 Dépôt requis
                  </div>
                ) : done ? (
      ...
      {/* Lucky Spin CTA */}
      <a href="/spin" className="flex items-center gap-4 rounded-[22px] border border-amber-200 bg-amber-50 p-4 afili-card transition active:scale-[0.99]">
        ...
        <div className="flex-1">
          ...
          <p className="text-[12px] text-slate-500">500 ou 800 FCFA à gagner · 1 tour par jour</p>
        </div>
      ...
      <div className="rounded-[20px] border border-slate-200 bg-white p-5 afili-card">
        ...
        <ul className="mt-3 space-y-2">
          {[
            ...
            "Telegram / TikTok : vérification manuelle par l'admin",
            ...
            <li key={i} className="flex items-start gap-2 text-[12px] text-slate-600">
              <span className="mt-0.5 shrink-0 font-black text-slate-400">·</span>{r}
            </li>
          ))}
        </ul>
      </div>
